...
class sqlite_database : public database_interface {
   ...
   */
  ~sqlite_database() override { close(); }

  // ムーブのみ許可
  sqlite_database(sqlite_database&& other) noexcept : db_(other.db_), error_msg_(std::move(other.error_msg_)) { other.db_ = nullptr; }
   ...
   * @brief エラーメッセージを取得する
   */
  auto error_message() -> std::string_view override { return error_msg_; }
